...
export default function App() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Create or update the user profile in Firestore for Google sign-up/sign-in
      try {
        await setDoc(doc(db, "users", cred.user.uid), {
          email: cred.user.email || "",
          displayName: cred.user.displayName || "مستخدم جوجل",
      ...
      setShowAuthModal(false);
      addToast("تم التسجيل/تسجيل الدخول بنجاح عبر Gmail!", "success");
    } catch (err: any) {
      console.error("Google Login Error:", err);
      let errMsg = "حدث خطأ أثناء تسجيل الدخول بجوجل.";
      if (err.code === "auth/popup-closed-by-user") {
        errMsg = "تم إلغاء تسجيل الدخول.";
      } else if (err.code === "auth/popup-blocked") {
        errMsg = "تم حظر نافذة تسجيل الدخول. يرجى السماح بالنوافذ المنبثقة وإعادة المحاولة.";
      }
      setAuthError(errMsg);
      addToast(errMsg, "error");
    } finally {
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="relative flex items-center py-2">
                ...
                <span className="flex-shrink-0 mx-4 text-xs text-light/50 font-serif">أو</span>
                <div className="flex-grow border-t border-gold/10"></div>
              </div>
              ...
              <button
                ...
                disabled={authLoading}
                className="w-full bg-white text-gray-900 font-bold py-3 rounded-xl transition-all duration-300 flex items-center justify-center gap-3 disabled:opacity-50 cursor-pointer shadow-lg hover:bg-gray-100"
              >
                ...
                <span>التسجيل/الدخول عبر Gmail</span>
              </button>
